...
def auth_start(session, secret: str) -> bool:
    # clean up untrusted input
    secret = secure_filename(secret)
    secret = secret.upper()

    # check if data exists
    tenant_folder = os.path.join(settings.BASE_DIR, "tenant_data", f"{secret}")
...
def auth_check(session) -> bool:
    if session.get("tenant_data"):
        return True
    return False


def auth_end(session) -> bool:
